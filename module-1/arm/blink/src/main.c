...
int main(void)
{
    ...
    //LED pins Output
    PyHal_GPIO_SetDir(H_pin,1);
    PyHal_GPIO_SetDir(P_pin,0);
    PyHal_GPIO_SetDir(Q_pin,0);
    PyHal_GPIO_SetDir(R_pin,0);
    PyHal_GPIO_SetDir(S_pin,0);

    while(1)
    {
	    H = !(Q&&S) || (!P&&S) || (P&&Q&&R) || (P&&!Q&&!R&&S);
        //Test GPIO Code
        PyHal_GPIO_Set(H_pin,H);
        PyHal_GPIO_Set(CLK_pin,0);
        HAL_DelayUSec(2000000);    
	PyHal_GPIO_Set(CLK_pin,1);
        HAL_DelayUSec(2000000);  
	P = PyHal_GPIO_Set(P_pin,P);
	Q = PyHal_GPIO_Set(Q_pin,Q);
	R = PyHal_GPIO_Set(R_pin,R);
	S = PyHal_GPIO_Set(S_pin,S);
    }
    /* Start the tasks and time running. */
    vTaskStartScheduler();
    dbg_str("\n");
